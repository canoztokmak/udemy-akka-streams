...
//    case Success(count) => println(s"total number of short strings is $count")
//    case Failure(th) => println(s"failed: $th")
//  }
  ...
  def enhanceFlow[A, B](flow: Flow[A, B, _]): Flow[A, B, Future[Int]] = {
    val genericCounter = Sink.fold[Int, B](0)((count, _) => count + 1)

    Flow.fromGraph(
      GraphDSL.create(genericCounter) { implicit builder => counterShape =>
        import GraphDSL.Implicits._
  ...
  enhancedFlowCountFuture.onComplete {
    case Success(value) => println(s"elements went through: $value")
